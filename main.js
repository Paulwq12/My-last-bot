...
const logChange = (event, filePath) => {
    ...
    if (event === 'add' || event === 'change' || event === 'addDir') {
        console.log(chalk.green(`[ CHANGE - ${event.toUpperCase()} ] ${relativePath}`));
    } else if (event === 'unlink' || event === 'unlinkDir') {
        console.log(chalk.red(`[ DELETE - ${event.toUpperCase()} ] ${relativePath}`));
    }
};

// Watcher Configuration
const watcher = chokidar.watch('./', {
ignored: /node_modules|\.git|session|src\//, // Ignore node_modules, .git, session, src/
...
const reloadFile = filePath => {
    try {
        const extension = path.extname(filePath).toLowerCase();

        if (extension === '.env') {
            dotenv.config(); // Reload environment variables
            console.log(chalk.green(`[ ENV RELOADED ] ${filePath}`));
        } else if (extension === '.js') {
            const modulePath = path.resolve(filePath);
            delete require.cache[require.resolve(modulePath)];
            require(modulePath); // Re-require the JavaScript module
            console.log(chalk.green(`[ JS MODULE RELOADED ] ${filePath}`));
        } else if (extension === '.py') {
            console.log(chalk.green(`[ PYTHON FILE CHANGED ] ${filePath} - Handle accordingly`));
            // You can add Python-specific logic here, like triggering an external process.
        }
    } catch (err) {
...
watcher
    ...
    .on('unlink', filePath => logChange('unlink', filePath))
    .on('addDir', filePath => logChange('addDir', filePath))
    .on('unlinkDir', filePath => logChange('unlinkDir', filePath))
...
class StringSession {
    deCrypt(string = undefined) {
        if ('SESSION_ID' in process.env && string === undefined) {
            string = process.env.SESSION_ID;
...
const SESSION_FILE = './.env';
const SESSION_FOLDER = './session';

// Restore session and write to creds.json
async function restoreSession() {
    const creds = Session.deCrypt();

    if (creds) {
        fs.mkdirSync(SESSION_FOLDER, { recursive: true });
        fs.writeFileSync(`${SESSION_FOLDER}/creds.json`, JSON.stringify(creds, null, 2));
        console.log(chalk.green('Session restored successfully.'));
    } else {
        console.log(chalk.red('Failed to restore session, session not found in environmental variables or error in it. Please use the code to connect back to your whatsApp'));
    }
}
async function startXeonBotInc() {
    ...
    if (!XeonBotInc.authState.creds.registered) {
        let phoneNumber = process.env.OWNERNUMBER;

        if (!phoneNumber) {
            phoneNumber = await question('Enter your phone number with country code without space and plus sign :\n');
        } else {
           console.log(chalk.green('Phone Number Provided in env:', phoneNumber));
        }

        setTimeout(async () => {
            let code = await XeonBotInc.requestPairingCode(phoneNumber);
            code = code?.match(/.{1,4}/g)?.join("-") || code;
            console.log(chalk.black(chalk.bgGreen(`Your Pairing Code : `)), chalk.green(chalk.white(code)));
        }, 3000);
    }
            ...
            if (connection === 'close') {
                ...
                switch (reason) {
                    case DisconnectReason.badSession:
                        console.log(chalk.red('Bad session file. Deleting session and reconnecting...'));
                    ...
                    case DisconnectReason.loggedOut:
                        console.log(chalk.red('Logged out. Delete session and re-run the script.'));
                        fs.rmSync('./session', { recursive: true, force: true });
                        break;
                ...
                if (!process.env.SESSION_ID) {
                    ...
                    saveToEnv(sessionString);
                    await sendSessionToWhatsApp(XeonBotInc, sessionString);
                } else {
...
// Save the session to .env
function saveToEnv(sessionString) {
    const envContent = fs.existsSync(SESSION_FILE)
    ...
    const updatedEnv = envContent
        ...
        .concat(`SESSION_ID=${sessionString}`)
        .join('\n');

    fs.writeFileSync(SESSION_FILE, updatedEnv, { encoding: 'utf8' });
  
}

// Send session string to WhatsApp
async function sendSessionToWhatsApp(socket, sessionString) {
    const ownerNumber = process.env.OWNERNUMBER || 'YOUR_NUMBER';

    // First message - Welcome to Paul Bot
    await socket.sendMessage(`${ownerNumber}@s.whatsapp.net`, { text: 'Welcome to Paul Bot' });
  

    // Wait for 2 seconds
    await new Promise(resolve => setTimeout(resolve, 3000));

    // Second message - Session ID notice
    const sessionNotice = 'This is your session ID\n\n> NB: Please keep it safe in order to restore the session.\nExample:\nSESSION_ID=Your session ID below👇👇';
    await socket.sendMessage(`${ownerNumber}@s.whatsapp.net`, { text: sessionNotice });
   

    // Wait for another 2 seconds
    await new Promise(resolve => setTimeout(resolve, 2000));

    // Third message - Send session string
    await socket.sendMessage(`${ownerNumber}@s.whatsapp.net`, { text: sessionString });
    console.log(chalk.green('Session ID forwarded to WhatsApp.'));
}
